...
| **Universal tithe** | Transparent, configurable, removable 10.0% humanitarian + 2.3% protocol routing of positive yield (`enforce_universal_tithe`) |
| **Jarzynski equality** | $\langle e^{-\beta W}\rangle = e^{-\beta\Delta F}$; recovers an equilibrium free-energy gap from non-equilibrium work |
| **Crooks fluctuation theorem** | $P_F(W)/P_R(-W)=e^{\beta(W-\Delta F)}$; relates forward/reverse work distributions |
| **Dissipated work** | $\langle W\rangle-\Delta F\ge0$; equals $k_BT\,D_{\mathrm{KL}}(P_F\|P_R)$ (irreversibility scale) |
| **Koopman operator** | Linear operator $(\mathcal K g)(x)=g(F(x))$ on observables of a nonlinear system |
| **EDMD** | Extended Dynamic Mode Decomposition: least-squares finite approximation $K=G_yG_x^+$ of $\mathcal K$ |
| **Artin braid group** | $B_n$, group of $n$-strand braids generated by crossings $\sigma_i$ |
| **Temperley-Lieb algebra** | $TL_n(\delta)$ planar-diagram algebra, $e_i^2=\delta e_i$; image of the braid skein map |
| **Kauffman bracket** | $\langle L\rangle$, regular-isotopy invariant via $\sigma_i\mapsto A\mathbf1+A^{-1}e_i$ |
| **Jones polynomial** | $V_L(t)=(-A^3)^{-w}\langle L\rangle$ at $t=A^{-4}$; isotopy invariant of a link |
| **Cross-impact ordering** | Minimising asymmetric impact cost $\sum_{a\,\text{before}\,b}L_{ab}$ over execution orderings |
